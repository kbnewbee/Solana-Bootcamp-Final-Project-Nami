...
mod mint_ticket {
	...
	fn test_transaction_is_ok() {
		...
		let (test_validator, fee_payer_info) = TestValidatorGenesis::default()
			.add_program("ticket", program_id)
			.start();
		let rpc_client = test_validator.get_rpc_client();

		let name: String = Default::default();
		let game: String = Default::default();
		let location: String = Default::default();
		let seat: String = Default::default();
		let price: f32 = Default::default();
		let date: String = Default::default();
		let class: String = Default::default();
		let available: u32 = Default::default();
		let is_owned: bool = Default::default();
		let is_gift: bool = Default::default();
		let is_used: bool = Default::default();

		let mint_ticket_info = Keypair::new();
		...
		let (ticket_pubkey, _) = Pubkey::find_program_address(
			&[b"ticket", mint_ticket_info.pubkey().as_ref()],
			&program_id,
		);
		let accounts = vec![
			AccountMeta::new(fee_payer_info.pubkey(), true),
			AccountMeta::new(mint_ticket_info.pubkey(), true),
			AccountMeta::new(ticket_pubkey, false),
			AccountMeta::new_readonly(system_program::id(), false),
			AccountMeta::new(mint_info.pubkey(), false),
		...
		let data = TicketInstruction::MintTicket(crate::generated::instructions::MintTicketArgs{
			name,
			game,
			location,
			seat,
		...
		create_account(&rpc_client, &fee_payer_info, &mint_info, &Pubkey::from_str("TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA").unwrap(), 82);
