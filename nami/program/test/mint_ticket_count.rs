...
mod mint_ticket_count {
	...
	fn test_transaction_is_ok() {
		...
		let mint_ticket_count_info = Keypair::new();
		...
		let (ticket_count_pubkey, _) = Pubkey::find_program_address(
			&[b"ticketCount", mint_ticket_count_info.pubkey().as_ref()],
			&program_id,
		);
		let accounts = vec![
			AccountMeta::new(fee_payer_info.pubkey(), true),
			AccountMeta::new(mint_ticket_count_info.pubkey(), true),
			AccountMeta::new(ticket_count_pubkey, false),
		...
		let data = TicketInstruction::MintTicketCount(crate::generated::instructions::MintTicketCountArgs{
			name,
			count,
		});
		let signers = vec![
			&fee_payer_info,
			&mint_ticket_count_info,
			&funding_info,
			&owner_info,
        ];
